...
- `input` uses the camelCase property names of the data classes/structs;
  each platform's test suite maps it onto the typed constructors.
...
Covered edge cases: pageView with/without screenName, cartSync full vs
empty-cart, productView multi-variant with nested `properties` /
